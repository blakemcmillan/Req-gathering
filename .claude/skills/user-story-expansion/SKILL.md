...
- **Open Questions Quarantine:** You must strictly ignore any sections labeled "Open Questions," "Gaps," "Deferred," or "Future Phases" within the PRD. You are explicitly forbidden from generating user stories, acceptance criteria, or assumptions to resolve these loose ends. If a feature or requirement depends on an unresolved open question, pause that sub-flow and omit it from the output entirely. Scope is restricted exclusively to confirmed, locked-in requirements.